...
class DontHitVesselLeft(smach.State):
    ...
    def check(self, veri):
      self.Lidar = {
        "left":  min(min(veri.ranges[0:135]),30),
        "right":  min(min(veri.ranges[225:360]),30),
        }
    def move_to_object(self, data):
        self.last_detection_recv = rospy.Time.now()
        if not data.isredfound or not data.isgreenfound:
        ...
        if self.min_left_angle > 0 and self.Lidar["right"]>5:
            self.m_l_a = True
            self.m_r_a = False
            tw.angular.z = 1/self.min_left_angle
        elif self.Lidar["Left"] > 5 and self.min_right_angle > 0:
            self.m_l_a = False
            self.m_r_a = True
            tw.angular.z = -1/self.min_right_angle
        elif self.Lidar["left"] > 30 and self.Lidar["right"]>30:
            self.m_l_a = False
            self.m_l_a = False
            tw.angular.z = 0.2
...
def main():
    ...
    with sm_top:
         ...
         smach.StateMachine.add('DONTHITVESSELLEFT', DontHitVesselLeft(),
                                transition={"FINISHED":"RECENTER", "ERROR":"ERROR"})
