...
class Yellow_follow(smach.State):
   def check(self, veri):
      self.Lidar = {
        ...
        "right":  min(min(veri.ranges[30:80]),30),
...
class Recenter(smach.State):
   def __init__(self):
      ...
      self.tw.angular.z = 0
      self.controller = PIDController(0.1, 0.000, 1.0 / 4.0)
      self.centerfound = False
      self.yellow = False
      self.black = False
      self.check1 = False
   ...
   def center(self,data):
      ...
      if not data.isredfound and data.isgreenfound:
         if Black.Blackfinished :
            self.tw.angular.z = self.controller.update(self.angular_z, 0.12,  1.0 / 30.0)
            #Rotate(10, 1 ,0.4)
         else:
            #Rotate(10, 2 ,0.4)
            self.tw.angular.z = self.controller.update(self.angular_z, -0.12, 1.0 / 30.0)
...
class Black(smach.State):
   ...
   def move_to_object(self,data):
      ...
      if self.Lidar["front-right"] < 1 or self.loopbegin :
         self.loopbegin = True
         ...
         if self.Lidar["right"] < 2:
            ...
            self.check1 = True
         if self.Lidar["right"] > 1 and self.check1:
            self.tw.linear.x = 0
