...
class Start(smach.State):
   ...
   def move_to_object(self, data):
      ...
      if self.Lidar["left"] < 4 or self.let:
         self.let = True
         if self.Lidar["left"] > 5:
            self.calculate = True
         return 
...
class Calculate(smach.State):
   ...
   def check(self, veri):
      self.Lidar = {
        "front-left":  min(min(veri.ranges[20:75]),30),
        "right":  min(min(veri.ranges[268:272]),30),
        "front-right":min(min(veri.ranges[280:340]),30),
        } 
      self.Lidarindex = {
        "front-left":  veri.ranges[20:75].index(min(veri.ranges[20:75])),
        "right":  min(min(veri.ranges[268:272]),30),
...
class Wait(smach.State):
   ...
   def check(self, veri):
      self.Lidar = {
        "front":  min(min(veri.ranges[0:30]),30,min(veri.ranges[340:360])),
        "right":  min(min(veri.ranges[260:280]),30),
        "front-right":min(min(veri.ranges[280:340]),30),
        } 
      self.move_to_object()
